...
if [ -d "$EXTRACTION_DIR" ]; then
    echo "The WordPress code is already extracted."
else
    # If not, unzip the file
    unzip "$ZIP_FILE"
    echo "WordPress code extracted"
fi
...
# Using the sed command find and replace in /var/www/html/wordpress/wp-config.php the placeholders on line 23, 26, and 29
